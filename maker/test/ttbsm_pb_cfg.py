...
process.pfIsolatedMuonsLoosePFlow = process.pfIsolatedMuonsPFlow.clone(
    combinedIsolationCut = cms.double(float(999.0)) 
)
...
process.InputMaker.input_type = cms.string("zprime")
...
process.filter_scraping = cms.Path(
    process.scrapingVeto
)

process.filter_hbhenoise = cms.Path(
    process.HBHENoiseFilter
)
